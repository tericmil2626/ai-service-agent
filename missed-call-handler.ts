...
import { createMissedCall, updateMissedCallTextBack, findCustomerByPhone, createCustomer, createJob } from './database.js';
import { sendSMS as sendTwilioSMS } from './twilio.js';
import { sendSMS as sendSignalWireSMS } from './signalwire-fetch.js';
...
const DEFAULT_TEXT_BACK_MESSAGE = (businessName: string) => 
  `Sorry we missed your call! This is ${businessName}. ` +
...
export class MissedCallHandler {
   ...
   */
  async handleCallStatus(data: MissedCallData): Promise<{
    handled: boolean;
    ...
    const missedCallId = await createMissedCall({
      customer_phone: from,
      business_phone: to,
      call_sid: callSid,
   ...
   */
  async handleTextBackReply(
    customerPhone: string,
    message: string,
    orchestrator: any
  ): Promise<{ success: boolean; response?: string; error?: string }> {
    // Ensure phone number has + prefix
    if (customerPhone && !customerPhone.startsWith('+')) customerPhone = '+' + customerPhone;
    ...
    try {
      ...
      await db.run(
        `UPDATE missed_calls SET converted_to_lead = 1 WHERE id = ?`,
        [missedCall.id]
      );
      ...
      if (customer) {
        const job = await db.get(
          'SELECT id FROM jobs WHERE customer_id = ? ORDER BY created_at DESC LIMIT 1',
          [customer.id]
        );
        if (job) {
          await updateMissedCallTextBack(missedCall.id, missedCall.text_back_message || this.textBackMessage, job.id);
        }
      }
