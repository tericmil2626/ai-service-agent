...
export function isAgentAvailable(agentId: string, tier: string): boolean {
  const config = getTierConfig(tier);
  return config.agents.includes(agentId);
}

export function isFeatureAvailable(feature: keyof TierConfig['features'], tier: string): boolean {
  const config = getTierConfig(tier);
  return config.features[feature];
}

export function getAvailableAgents(tier: string): string[] {
  return getTierConfig(tier).agents;
}

export function checkLimit(limitType: keyof TierConfig['limits'], current: number, tier: string): boolean {
  const config = getTierConfig(tier);
