import { getDb, createJob, updateJobStatus, saveMessage } from './database';
...
import { ReviewRequestAgent } from './agents/ReviewRequestAgent';
...
export class ServiceBusinessOrchestrator {
  ...
  private reviewAgent: ReviewRequestAgent;
  ...
  constructor() {
    ...
    this.reviewAgent = new ReviewRequestAgent();
    this.knowledgeBaseAgent = new KnowledgeBaseAgent();
  }

  async processIncomingMessage(incoming: IncomingMessage): Promise<OrchestratorResponse> {
    ...
    const intakeResult = await this.intakeAgent.handleMessage(
      incoming.message,
  ...
  async handleAgentResponse(
    ...
    switch (agentType) {
      case 'scheduling':
        // Pass job data directly to handleTimeSelection for stateless operation
  ...
  async handleKnowledgeQuery(customerId: number, message: string, channel: string): Promise<OrchestratorResponse> {
  ...
  private async getActiveJob(customerId: number): Promise<any | null> {
    const db = await getDb();
    return db.get(
      'SELECT * FROM jobs WHERE customer_id = ? AND status NOT IN ("completed", "cancelled", "closed") ORDER BY created_at DESC LIMIT 1',
      customerId
    );
  }

  private async routeToActiveAgent(job: any, incoming: IncomingMessage): Promise<OrchestratorResponse> {
    ...
    switch (job.status) {
      ...
      case 'qualified':
        // Ready for scheduling - handle time selection
        return this.handleAgentResponse(job.id, 'scheduling', incoming.message);

      case 'new':
        // Check if job has all required info (intake complete but status not updated)
        if (job.service_type && job.description && job.urgency) {
          // Update status to qualified and proceed to scheduling
          await updateJobStatus(job.id, 'qualified');
          return this.handleAgentResponse(job.id, 'scheduling', incoming.message);
        }
        // Continue with intake
        return this.processIncomingMessage(incoming);
  ...
  private async sendAppointmentReminders(): Promise<void> {
    ...
    const appointments = await db.all(
      'SELECT * FROM appointments WHERE scheduled_date = ? AND status = "confirmed" AND reminder_sent = 0',
      dateStr
    );
  ...
  private async dispatchUnassignedJobs(): Promise<void> {
    const db = await getDb();
    
    const unassignedJobs = await db.all(
      'SELECT * FROM appointments WHERE status = "confirmed" AND technician_id IS NULL'
    );
  ...
  private async getJobDataForReview(jobId: number): Promise<any> {
    const db = await getDb();
    return db.get(`
      SELECT 
        j.id as job_id,
  ...
  async handleMissedAppointment(appointmentId: number): Promise<void> {
