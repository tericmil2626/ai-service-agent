...
export class LeadGenerationAgent {
  private serviceKeywords: Record<string, string[]> = {
  ...
  private urgencyPatterns: Record<string, RegExp[]> = {
    'high': [
  ...
  async analyzeSource(rawLead: RawLead): Promise<EnrichedLead | null> {
  ...
  async processBatch(rawLeads: RawLead[]): Promise<{
    highPriority: EnrichedLead[];
    mediumPriority: EnrichedLead[];
    lowPriority: EnrichedLead[];
    rejected: number;
  }> {
  ...
  private detectServiceCategory(text: string): string | null {
  ...
  private extractContactInfo(text: string): { name?: string; phone?: string; email?: string } {
  ...
  private inferLocation(metadata?: any): string | undefined {
  ...
  private extractProblemDescription(text: string, serviceCategory: string): string {
  ...
  private calculateConfidence(
    ...
    if (serviceCategory) score += 30;

    // Has contact info: +20 each
    if (contactInfo.phone) score += 20;
    ...
    if (problemDescription && problemDescription.length > 10) score += 10;

    return Math.min(score, 100);
  }

  // Handoff recommendations
  getHandoffRecommendation(lead: EnrichedLead): {
    agent: string;
    urgency: string;
    action: string;
  } {
  ...
  getStructuredOutput(leads: EnrichedLead[]): any {
    return {
...
export default LeadGenerationAgent;
