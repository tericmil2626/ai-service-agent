...
async function mockLLMResponse(messages: any[]): Promise<string> {
  ...
  const isSchedulingRequest = messages.some(m => 
    m.role === 'system' && m.content.includes('scheduling assistant')
  );
  ...
  const isTimeParsing = messages.some(m => 
    m.role === 'system' && (m.content.includes('Parse the customer') || m.content.includes('selected slot'))
  );
...
export async function generateResponse<T extends z.ZodType>(
  ...
  const messagesWithSchema = messages.map((m, i) =>
    i === 0 && m.role === 'system'
 ...
 */
export async function extractEntities(message: string, existingData: Record<string, any> = {}) {
  const schema = z.object({
    name: z.union([z.string(), z.null()]).transform(val => typeof val === 'string' ? val : null).nullable().default(null).describe('Customer name if mentioned'),
    phone: z.union([z.string(), z.null()]).transform(val => typeof val === 'string' ? val : null).nullable().default(null).describe('Phone number if mentioned'),
    address: z.union([z.string(), z.object({}).passthrough(), z.null()])
      .transform(val => {
    ...
    service_type: z.union([z.enum(['plumbing', 'electrical', 'hvac', 'appliance', 'other', 'unknown']), z.null()])
      .transform(val => val === null ? 'unknown' : val)
      .default('unknown')
      .describe('Type of service needed'),
    problem_description: z.union([z.string(), z.null()]).transform(val => typeof val === 'string' ? val : null).nullable().default(null).describe('Description of the problem'),
    urgency: z.union([z.enum(['high', 'medium', 'low', 'unknown']), z.null()])
      .transform(val => val === null ? 'unknown' : val)
      .default('unknown')
      .describe('Urgency level based on problem description'),
    preferred_time: z.string().nullable().default(null).describe('When customer wants service (e.g., today, tomorrow, morning)'),
    is_disqualified: z.boolean().default(false).describe('True if this is spam, wrong number, or outside service area'),
    disqualify_reason: z.string().nullable().default(null).describe('Why this lead is disqualified'),
    missing_info: z.union([z.array(z.string()), z.record(z.any())]).transform(val => {
    ...
    sentiment: z.union([z.enum(['urgent', 'frustrated', 'neutral', 'happy']), z.null()])
    ...
    is_confirmation: z.boolean().default(false).describe('True if customer is saying yes, ok, sure, confirming, or agreeing to proceed'),
  });
...
export async function generateSchedulingResponse(
  ...
  const schema = z.object({
    response: z.union([z.string(), z.object({ message: z.string() }).transform(obj => obj.message)])
      .describe('Natural language response presenting the options'),
    slot_references: z.union([z.array(z.number()), z.array(z.string()), z.null(), z.undefined()])
      .transform(val => {
        if (!val) return [];
...
export async function parseTimeSelection(
  ...
  const schema = z.object({
    selected_slot_index: z.union([z.number(), z.null(), z.undefined()])
      .transform(val => val === undefined ? null : val)
      .describe('Index of selected slot (0-based), or null if unclear'),
    selected_slot: z.union([z.number(), z.null(), z.undefined()])
      .transform(val => val === undefined ? null : val)
      .describe('Alternative field for slot index'),
    confidence: z.union([z.number(), z.undefined()]).default(0.5).describe('Confidence level 0-1'),
    clarification_needed: z.union([z.boolean(), z.undefined()]).default(false).describe('True if we need to ask for clarification'),
  }).transform(obj => ({
    ...obj,
