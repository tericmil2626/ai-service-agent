...
import { dbGet, dbAll, dbRun, updateJobStatus } from '../database.js';
...
export class DispatchAgent implements ServiceAgent {
  id = 'dispatch';
  name = 'Dispatch Agent';

  private dispatchData: DispatchData | null = null;
  ...
  async handleMessage(message: string, context: MessageContext): Promise<AgentResponse> {
  ...
  private async assignTechnician(): Promise<AgentResponse> {
    if (!this.dispatchData) {
      return {
    ...
    try {
      ...
      await updateJobStatus(this.dispatchData.job_id, 'dispatched');
  ...
  private async getActiveTechnicians(): Promise<Technician[]> {
  ...
  private selectTechnicianRoundRobin(technicians: Technician[]): Technician {
  ...
  private async updateAppointmentWithTechnician(technicianId: number): Promise<void> {
    ...
    await dbRun(
      `UPDATE appointments 
       ...
       WHERE id = ?`,
      [technicianId, this.dispatchData.appointment_id]
    );

    console.log(`[DispatchAgent] Assigned technician ${technicianId} to appointment ${this.dispatchData.appointment_id}`);
  }

  private async notifyCustomer(technician: Technician): Promise<void> {
    if (!this.dispatchData) return;
  ...
  setState(state: Record<string, any>): void {
    if (state.dispatchData) {
      this.dispatchData = state.dispatchData as DispatchData;
    }
