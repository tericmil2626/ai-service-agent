...
import { getDb, dbGet, dbAll, dbRun, updateJobStatus } from '../database.js';
import { generateText } from '../llm.js';
import { getCalendarService, CalendarEvent } from '../calendar.js';
...
export class DispatchAgent {
  private data: DispatchData | null = null;
  private availableTechnicians: Technician[] = [];
  private initialized: boolean = false;

  /**
   * Initialize the dispatch agent with appointment data from scheduling
   */
  async initialize(data: DispatchData): Promise<void> {
    this.data = data;
   ...
   */
  async receiveFromScheduling(appointmentData: DispatchData): Promise<DispatchResult> {
   ...
   */
  async assignTechnician(): Promise<DispatchResult> {
    if (!this.data) {
    ...
    if (this.availableTechnicians.length === 0) {
      ...
      await updateJobStatus(this.data.job_id, 'pending_dispatch');
    ...
    await updateJobStatus(this.data.job_id, 'dispatched');
   ...
   */
  private async findTechniciansBySpecialty(serviceType: string): Promise<Technician[]> {
    const db = await getDb();
    
    // Get all active technicians
    const rows = await dbAll(
      `SELECT id, name, phone, email, specialties, is_active 
       FROM technicians 
       WHERE is_active = 1`
    );
   ...
   */
  private async rankTechnicians(technicians: Technician[]): Promise<Technician[]> {
    if (!this.data) return technicians;
    ...
    const workloadMap = new Map(workloads.map((w: any) => [w.technician_id, w.job_count]));

    // Score each technician
    const scored = technicians.map(tech => {
      const workload = workloadMap.get(tech.id) || 0;
      const specialtyMatch = tech.specialties.filter(s => 
        this.data!.service_type.toLowerCase().includes(s.toLowerCase())
      ).length;
   ...
   */
  private async assignTechnicianToAppointment(technicianId: number, technician: Technician): Promise<void> {
    if (!this.data) return;
    ...
    await dbRun(
      `UPDATE appointments 
       ...
       WHERE id = ?`,
      [technicianId, this.data.appointment_id]
    );
      ...
      if (calendarService.isInitialized()) {
        ...
        if (eventId) {
          ...
          await dbRun(
            ...
            [eventId, this.data.appointment_id]
          );
   ...
   */
  private async updateAppointmentStatus(status: string): Promise<void> {
    if (!this.data) return;
    ...
    await dbRun(
      `UPDATE appointments 
       ...
       WHERE id = ?`,
      [status, this.data.appointment_id]
    );
  }
   ...
   */
  private async generateTechnicianNotification(technician: Technician): Promise<string> {
    if (!this.data) return '';
...
Reply YES to confirm or NO to decline.`;
   ...
   */
  async handleTechnicianResponse(response: string): Promise<DispatchResult> {
    if (!this.data) {
      return { response: "Error: No appointment data available.", assigned: false };
    }
   ...
   */
  async markAsDispatched(): Promise<DispatchResult> {
    if (!this.data) {
      return { response: "Error: No appointment data available.", assigned: false };
    }
    ...
    await updateJobStatus(this.data.job_id, 'in_progress');
   ...
   */
  getStatus(): { data: DispatchData | null; technicians: Technician[]; initialized: boolean } {
    return {
      data: this.data,
  ...
  getState(): { data: DispatchData | null; availableTechnicians: Technician[]; initialized: boolean } {
    return {
      data: this.data,
  ...
  setState(state: { data: DispatchData | null; availableTechnicians: Technician[]; initialized: boolean }): void {
...
export async function createTechnician(data: {
  ...
  const result = await dbRun(
    `INSERT INTO technicians (name, phone, email, specialties, is_active)
     VALUES (?, ?, ?, ?, 1)`,
    [data.name, data.phone, data.email || null, JSON.stringify(data.specialties)]
  );
  return result.lastID;
}

export async function getTechnicianById(id: number): Promise<Technician | null> {
  const row = await dbGet(
    `SELECT id, name, phone, email, specialties, is_active FROM technicians WHERE id = ?`,
    [id]
  );
...
export async function getAllTechnicians(): Promise<Technician[]> {
  const rows = await dbAll(
    `SELECT id, name, phone, email, specialties, is_active FROM technicians WHERE is_active = 1`
  );
