...
const DispatchAgent_js_1 = require("./src/agents/DispatchAgent.js");
...
async function testDispatchAgent() {
    ...
    const techs = await (0, database_js_1.dbAll)('SELECT id, name, specialties FROM technicians WHERE is_active = 1');
    ...
    let customer = await (0, database_js_1.dbGet)('SELECT id FROM customers WHERE phone = ?', ['+15559998888']);
    if (!customer) {
        const result = await (0, database_js_1.dbRun)('INSERT INTO customers (name, phone, address) VALUES (?, ?, ?)', ['Test Customer', '+15559998888', '123 Test Street, Springfield, IL']);
    ...
    // Create a test job
    const jobResult = await (0, database_js_1.dbRun)('INSERT INTO jobs (customer_id, service_type, description, urgency, status) VALUES (?, ?, ?, ?, ?)', [customer.id, 'hvac', 'AC not working, house is 90 degrees', 'high', 'scheduled']);
    ...
    const apptResult = await (0, database_js_1.dbRun)('INSERT INTO appointments (job_id, scheduled_date, scheduled_time, status) VALUES (?, ?, ?, ?)', [jobId, dateStr, '10:00', 'confirmed']);
    ...
    console.log();
    // Verify in database
    const assigned = await (0, database_js_1.dbGet)(`SELECT a.*, t.name as tech_name, t.phone as tech_phone
     FROM appointments a
    ...
    await (0, database_js_1.dbRun)('DELETE FROM appointments WHERE id = ?', [appointmentId]);
    await (0, database_js_1.dbRun)('DELETE FROM jobs WHERE id = ?', [jobId]);
