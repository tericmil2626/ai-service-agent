...
class SchedulingAgent {
    ...
    async handleTimeSelection(selection) {
        ...
        // Update job status
        await (0, database_1.updateJobStatus)(this.data.job_id, 'scheduled');
        // Format confirmation
    ...
    async handleRescheduleRequest(currentAppointmentId, newPreference) {
        ...
        const current = await db.get(`SELECT a.*, j.service_type, c.name, c.address 
       FROM appointments a
    ...
    async handleCancellation(appointmentId) {
        const db = await (0, database_1.getDb)();
        try {
            await db.run(`UPDATE appointments SET status = 'cancelled', updated_at = CURRENT_TIMESTAMP WHERE id = ?`, appointmentId);
            // Also update job status
            const appointment = await db.get('SELECT job_id FROM appointments WHERE id = ?', appointmentId);
            if (appointment) {
                await (0, database_1.updateJobStatus)(appointment.job_id, 'cancelled');
            }
            return {
    ...
    async getAvailableSlots(urgency) {
        ...
        for (let day = 0; day < daysToCheck; day++) {
            ...
            const bookedSlots = await db.all(`SELECT scheduled_time FROM appointments 
    ...
    parseTimeSelection(selection) {
        ...
        if (numberMatch) {
            let index;
            if (lower.includes('first') || lower.includes('1'))
                index = 0;
    ...
    async bookAppointment(slot) {
        if (!this.data)
            return null;
        try {
            const appointmentId = await (0, database_1.createAppointment)({
...
exports.default = SchedulingAgent;
