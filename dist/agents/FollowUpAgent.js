...
class FollowUpAgent {
    ...
    async handleResponse(leadId, response) {
        ...
        if (positiveIndicators.some(ind => lowerResponse.includes(ind))) {
            await (0, database_1.updateJobStatus)(leadId, 'ready_to_schedule');
            return {
                action: 'schedule',
        ...
        if (negativeIndicators.some(ind => lowerResponse.includes(ind))) {
            await (0, database_1.updateJobStatus)(leadId, 'closed');
            return {
                action: 'close',
    ...
    `, appointmentId);
        ...
        await db.run('UPDATE appointments SET status = ? WHERE id = ?', ['no_show', appointmentId]);
        await (0, database_1.saveMessage)({
            customer_id: appointment.customer_id,
            job_id: appointment.job_id,
            channel: 'sms',
    ...
    async sendPostServiceFollowUp(jobId) {
        const db = await (0, database_1.getDb)();
    ...
    determineStage(lastContactDate, status) {
        if (!lastContactDate)
            return 'initial';
        const daysSince = Math.floor((Date.now() - new Date(lastContactDate).getTime()) / (1000 * 60 * 60 * 24));
        if (daysSince >= this.schedule.final)
            return 'final';
        if (daysSince >= this.schedule.second)
            return 'second';
        if (daysSince >= this.schedule.initial)
            return 'initial';
        return 'initial';
    }
    isTimeForFollowUp(lead) {
        if (!lead.last_contact_date)
            return true;
        const daysSince = Math.floor((Date.now() - new Date(lead.last_contact_date).getTime()) / (1000 * 60 * 60 * 24));
    ...
    async updateLeadStatus(leadId, stage) {
        const db = await (0, database_1.getDb)();
        await db.run(`UPDATE jobs SET 
        status = ?, 
        updated_at = CURRENT_TIMESTAMP 
      WHERE id = ?`, ['awaiting_response', leadId]);
    }
    getStructuredOutput(lead, action) {
        const base = {
            name: lead.name,
            phone: lead.phone,
...
exports.FollowUpAgent = FollowUpAgent;
exports.default = FollowUpAgent;
