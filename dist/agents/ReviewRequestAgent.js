...
exports.ReviewRequestAgent = void 0;
const database_1 = require("../database");
class ReviewRequestAgent {
    constructor() {
        this.maxRequests = 2;
        this.requestDelayHours = 3; // Initial request 3 hours after completion
        this.followUpDelayHours = 24; // Follow-up after 24 hours if no response
    }
    async checkJobsForReviewRequest() {
    ...
    async sendSatisfactionCheck(data) {
    ...
    async handleSatisfactionResponse(data, response) {
    ...
    async recordReviewReceived(jobId, platform) {
        const db = await (0, database_1.getDb)();
        await db.run(`
      UPDATE jobs SET 
        review_received = 1,
    ...
    generateSatisfactionCheck(data) {
    ...
    async updateJobReviewStatus(jobId, status) {
        const db = await (0, database_1.getDb)();
        await db.run(`
      UPDATE jobs SET 
        review_status = ?,
    ...
    async confirmSatisfaction(jobId) {
        const db = await (0, database_1.getDb)();
        await db.run(`
      UPDATE jobs SET 
    ...
    async incrementReviewRequestCount(jobId) {
        const db = await (0, database_1.getDb)();
        await db.run(`
      UPDATE jobs SET 
    ...
    getStructuredOutput(data, status, platform) {
        const base = {
            name: data.name,
            phone: data.phone,
...
exports.ReviewRequestAgent = ReviewRequestAgent;
exports.default = ReviewRequestAgent;
