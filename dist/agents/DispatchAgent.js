...
class DispatchAgent {
    ...
    async receiveScheduledJob(jobData) {
    ...
    async handleTechnicianDelay(jobId, delayMinutes) {
        const db = await (0, database_1.getDb)();
        // Get job and current assignment
        const job = await db.get(`SELECT a.*, j.service_type, c.name as customer_name, c.phone, c.address, t.name as technician_name
       FROM appointments a
       JOIN jobs j ON a.job_id = j.id
       JOIN customers c ON j.customer_id = c.id
       JOIN technicians t ON a.technician_id = t.id
       WHERE a.job_id = ? AND a.status IN ('confirmed', 'dispatched')`, jobId);
        if (!job) {
            return {
        ...
        await db.run(`UPDATE appointments SET status = 'delayed', notes = ?, updated_at = CURRENT_TIMESTAMP WHERE job_id = ?`, [`Delayed by ${delayMinutes} minutes`, jobId]);
    ...
    async handleTechnicianReassignment(jobId, reason) {
        const db = await (0, database_1.getDb)();
        // Get job details
        const job = await db.get(`SELECT j.*, c.name, c.phone, c.address, a.scheduled_date, a.scheduled_time
       FROM jobs j
       JOIN customers c ON j.customer_id = c.id
       ...
       WHERE j.id = ?`, jobId);
        ...
        // Update assignment
        await db.run(`UPDATE appointments SET technician_id = ?, status = 'rescheduled', updated_at = CURRENT_TIMESTAMP WHERE job_id = ?`, [newTech.id, jobId]);
        const assignment = {
    ...
    async notifyTechnicianEnRoute(jobId) {
        const db = await (0, database_1.getDb)();
        const job = await db.get(`SELECT j.*, c.name as customer_name, c.address, c.phone, t.name as technician_name, a.scheduled_time
       FROM jobs j
       JOIN customers c ON j.customer_id = c.id
       JOIN appointments a ON j.id = a.job_id
       JOIN technicians t ON a.technician_id = t.id
       WHERE j.id = ?`, jobId);
        ...
        // Update status
        await db.run(`UPDATE appointments SET status = 'dispatched', updated_at = CURRENT_TIMESTAMP WHERE job_id = ?`, jobId);
    ...
    async getTechnicianSchedule(technicianId, date) {
        const db = await (0, database_1.getDb)();
        return db.all(`SELECT a.*, j.service_type, c.name as customer_name, c.address
       FROM appointments a
       JOIN jobs j ON a.job_id = j.id
       JOIN customers c ON j.customer_id = c.id
       WHERE a.technician_id = ? AND a.scheduled_date = ? AND a.status != 'cancelled'
       ORDER BY a.scheduled_time`, [technicianId, date]);
    }
    async getQualifiedTechnicians(serviceType) {
        const db = await (0, database_1.getDb)();
        ...
        const technicians = await db.all('SELECT * FROM technicians WHERE is_active = 1');
    ...
    async selectBestTechnician(technicians, job) {
        if (technicians.length === 0)
            return null;
        ...
        const techWorkloads = await Promise.all(technicians.map(async (tech) => {
            const count = await db.get('SELECT COUNT(*) as count FROM appointments WHERE technician_id = ? AND scheduled_date = ? AND status IN ("confirmed", "pending")', [tech.id, jobDate]);
    ...
    async assignTechnicianToJob(jobId, technicianId) {
        const db = await (0, database_1.getDb)();
        // Update appointment with technician
        await db.run(`UPDATE appointments SET technician_id = ?, status = 'confirmed', updated_at = CURRENT_TIMESTAMP WHERE job_id = ?`, [technicianId, jobId]);
        // Update job status
        await (0, database_1.updateJobStatus)(jobId, 'scheduled');
    }
    generateCustomerNotification(assignment) {
        if (!this.jobData)
            return '';
    ...
    getStructuredOutput() {
        if (!this.jobData || !this.assignedTechnician)
            return null;
        return {
