...
class LeadQualificationAgent {
    constructor(channel = 'sms') {
        this.data = { lead_status: 'in_progress' };
    ...
    async extractInformation(message, phoneNumber) {
    ...
    async checkDisqualified(message) {
        const lowerMsg = message.toLowerCase();
    ...
    async persistToDatabase() {
        ...
        if (!this.data.customer_id) {
            const customer = await (0, database_1.findOrCreateCustomer)({
                name: this.data.name,
        ...
        if (this.data.service_type && !this.data.job_id) {
            const jobId = await (0, database_1.createJob)({
    ...
    async persistConversation(inbound, outbound) {
        if (!this.data.customer_id)
            return;
    ...
    async finalizeLead() {
        if (this.data.job_id) {
            await (0, database_1.updateJobStatus)(this.data.job_id, 'qualified');
        }
...
exports.default = LeadQualificationAgent;
